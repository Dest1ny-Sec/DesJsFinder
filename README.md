...
> ⚠️ **仅供安全研究使用 · 使用前请确保获得目标授权 · 严禁用于未授权测试或盈利行为**

---

## 它不是普通的 JS 扫描器，而是一个**被动采集的浏览器扩展**

传统 JS 扫描器跑一次主动探测就触发 WAF、被封 IP。DesJsFinder 走完全不同的路 —— **装到浏览器里，安静地等用户正常浏览**。扩展在后台静默抓取所有 JS 文件，自动提取 API / 密钥 / 指纹，全程目标无感知。

- 装上扩展后正常浏览目标网站，扩展在后台**静默抓取**所有加载的 JS 文件
- 自动提取 **API endpoint / 密钥 / Token / 内网 URL / 技术指纹**
- 跟传统主动扫描不同：**被动视角 = 不触发 WAF，不留痕**，目标无感知
